...
type Props = { nftCount: number };

export default function ImagingRecords({ nftCount = 0 }: Props) {
  const items = [
    { title: "X-Ray I",     desc: "≥ 5 NFTs",    ok: nftCount >= 5 },
    { title: "X-Ray II",    desc: "≥ 10 NFTs",   ok: nftCount >= 10 },
    { title: "X-Ray III",   desc: "≥ 20 NFTs",   ok: nftCount >= 20 },
    { title: "CT Scan",     desc: "≥ 50 NFTs",   ok: nftCount >= 50 },
    { title: "MRI Records", desc: "≥ 100 NFTs",  ok: nftCount >= 100 },
    { title: "Full Body",   desc: "≥ 500 NFTs",  ok: nftCount >= 500 },
  ];
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-black/20 overflow-hidden h-full flex flex-col">
      ...
      <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between">
        <div className="text-[15px] font-semibold">Imaging Records</div>
        <div className="text-sm opacity-70">NFTs: {nftCount}</div>
      </div>
