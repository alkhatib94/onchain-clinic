...
function isHttpUrl(raw?: string) {
  try { if (!raw) return false; const u = new URL(raw); return u.protocol === "http:" || u.protocol === "https:"; }
  catch { return false; }
}
...
function asArr<T = any>(x: any): T[] { return Array.isArray(x) ? (x as T[]) : []; }
...
async function prices() {
  ...
  if (key) { headers["x-cg-demo-api-key"] = key; headers["x-cg-pro-api-key"] = key; }
  try {
...
export async function GET(req: NextRequest) {
  try {
    if (req.nextUrl.searchParams.get("debug") === "ping") {
      return new Response(JSON.stringify({ ok:true, note:"alive" }), { headers: { "content-type":"application/json" }});
    }
    ...
    // ===== 1) جرّب BaseScan =====
    const [bsTx, bsE20, bsNFT] = await Promise.all([
    ...
    let normal = asArr<any>(bsTx?.result);
    let erc20  = asArr<any>(bsE20?.result);
    let erc721 = asArr<any>(bsNFT?.result);

    // حدّد إن كانت BaseScan “غير مفيدة”: NOTOK / rate-limit / result ليس Array
    const basescanUseful =
      Array.isArray(bsTx?.result) || Array.isArray(bsE20?.result) || Array.isArray(bsNFT?.result);

    // ===== 2) Fallback إلى Etherscan V2 إذا BaseScan غير مفيد أو كله صفر =====
    let provider: "basescan" | "etherscan-v2" = "basescan";
    if (!basescanUseful || (normal.length === 0 && erc20.length === 0 && erc721.length === 0)) {
      const [exTx, exE20, exNFT] = await Promise.all([
        etherscanV2({ module:"account", action:"txlist",       address, startblock:"0", endblock:"99999999", page:"1", offset:"10000", sort:"asc" }) || [],
        etherscanV2({ module:"account", action:"tokentx",      address, startblock:"0", endblock:"99999999", page:"1", offset:"10000", sort:"asc" }) || [],
        etherscanV2({ module:"account", action:"tokennfttx",   address, startblock:"0", endblock:"99999999", page:"1", offset:"10000", sort:"asc" }) || [],
      ]);
      if ((exTx as any[])?.length || (exE20 as any[])?.length || (exNFT as any[])?.length) {
        normal = asArr<any>(exTx); erc20 = asArr<any>(exE20); erc721 = asArr<any>(exNFT);
    ...
    for (const t of ok) { const ts = Number(t.timeStamp); days.add(isoDay(ts)); weeks.add(isoWeek(ts)); months.add(isoMonth(ts)); }
    ...
    // العقود
    const toSet = new Set<string>(); for (const t of ok) if (t.to) toSet.add(String(t.to).toLowerCase());
    const toArr = [...toSet]; const flags = await Promise.all(toArr.map((a) => isContract(a)));
    const cset  = new Set<string>(); for (let i=0;i<toArr.length;i++) if (flags[i]) cset.add(toArr[i]);
    ...
    for (const k of Object.keys(THIRD_PARTY_BRIDGES)) thirdPartySets[k] = new Set((THIRD_PARTY_BRIDGES as any)[k].contracts || []);
    const bridgeCounts: Record<string, number> = {}; for (const k of Object.keys(thirdPartySets)) bridgeCounts[k] = 0;

    for (const t of ok) {
      const from = String(t.from || "").toLowerCase(); const to = String(t.to || "").toLowerCase();
      const val  = Number(t.value) / 1e18;
      if (to === address) {
        if (NATIVE_BRIDGE_SENDERS_L2.includes(from)) { depositedEth += val; nativeBridgeUsed = true; }
        for (const b of Object.keys(thirdPartySets)) {
          if (thirdPartySets[b].has(from)) { depositedEth += val; usedThirdPartyBridge = true; bridgeCounts[b] += 1; }
        }
      }
    }
    ...
    // balance (من BaseScan؛ ولو ما نفع نجرب من Etherscan V2)
    let balanceEth = 0;
    const bsBal = await basescan({ module:"account", action:"balance", address, tag:"latest" });
    if (bsBal && typeof bsBal.result !== "undefined") {
      const wei = Number(String(bsBal.result ?? "0")); if (!Number.isNaN(wei)) balanceEth = wei / 1e18;
    } else {
      const exBal = await etherscanV2({ module:"account", action:"balance", address, tag:"latest" });
      const wei = Number(String((exBal as any)?.balance ?? exBal ?? "0"));
      if (!Number.isNaN(wei)) balanceEth = wei / 1e18;
    }

    // ===== Usage (swaps) =====
    const DEX_KEYS: (keyof typeof PROTOCOL_ADDRS)[] = ["uniswap","pancake","sushi","aerodrome","matcha"];
    const DEX_ROUTER_SET = new Set<string>(); for (const k of DEX_KEYS) for (const a of PROTOCOL_ADDRS[k]) DEX_ROUTER_SET.add(a.toLowerCase());
    const SWAP_WORDS = Array.from(new Set<string>([
      "swap","exactinput","exactoutput","universalrouter","v3swap",
      ...PROTOCOL_KEYWORDS.uniswap, ...PROTOCOL_KEYWORDS.pancake, ...PROTOCOL_KEYWORDS.sushi, ...PROTOCOL_KEYWORDS.aerodrome, ...PROTOCOL_KEYWORDS.matcha
    ].map(s=>s.toLowerCase())));
    ...
    for (const tr of erc20) {
      const h = String(tr.hash || tr.transactionHash || ""); if (!h) continue;
      const from = String(tr.from || "").toLowerCase(); const to = String(tr.to || "").toLowerCase();
      const dec = Number(tr.tokenDecimal || 6); const sym = String(tr.tokenSymbol || "").toUpperCase(); const val = Number(tr.value || 0);
    ...
    const looksLikeSwap = (t: any) => {
      ...
      const to = String(t.to || "").toLowerCase(); const from = String(t.from || "").toLowerCase();
      return SWAP_WORDS.some(w=>fn.includes(w)) || DEX_ROUTER_SET.has(to) || DEX_ROUTER_SET.has(from);
    };

    const okByHash: Record<string, any> = {};
    for (const t of ok) { const h = String(t.hash || ""); if (h) okByHash[h] = t; if (looksLikeSwap(t) && h) swapHashes.add(h); }
    ...
    for (const h of swapHashes) {
      const t = okByHash[h]; const ethUsd = t ? (Number(t.value || 0) / 1e18) * ethPrice : 0;
      const e20 = byHash[h]; const stableUsd = e20?.stableUsd || 0; const anyUsd = e20?.anyUsd || 0;
    ...
    const gasEth = ok.reduce((s: number, t: any) => {
      const gu = Number(t.gasUsed || t.gas || 0); const gp = Number(t.gasPrice || 0);
      return s + (gu * gp) / 1e18;
    }, 0);

    const deployedContracts = ok.filter((t: any) => {
      const to = String(t.to || "").toLowerCase(); const created = String((t as any).contractAddress || "");
    ...
    function countProto(name: keyof typeof PROTOCOL_ADDRS) {
      ...
      const words = PROTOCOL_KEYWORDS[name]; let c = 0;
      for (const t of allTxsForProto) { const byAddr = addrs.size>0 && (addrs.has(t.to) || addrs.has(t.from)); const byWord = words.some((w)=>t.fn.includes(w)); if (byAddr || byWord) c++; }
      return c;
    }
    ...
    return new Response(JSON.stringify({
      source: {
        provider,
        basescan: {
          tx_status: bsTx?.status,  tx_msg: bsTx?.message,  tx_len: Array.isArray(bsTx?.result) ? bsTx?.result.length : -1,
          e20_status: bsE20?.status, e20_msg: bsE20?.message, e20_len: Array.isArray(bsE20?.result) ? bsE20?.result.length : -1,
          nft_status: bsNFT?.status, nft_msg: bsNFT?.message, nft_len: Array.isArray(bsNFT?.result) ? bsNFT?.result.length : -1,
        },
      },
      ...
      nativeTxs, tokenTxs, walletAgeDays,
      uniqueDays, uniqueWeeks, uniqueMonths,
      ...
      // Usage
      swaps, stablecoinTxs, usdcTrades, stablecoinTypes, maxSwapUsd,

      // Variety
      erc20Count, nftCount,

      // Dosage / Costs
      totalVolumeEth, gasEth,
      ...
      // Clinics
      uniswap, sushi, pancake, aerodrome, aave, limitless, stargate, metamask, matcha, lendingAny,
